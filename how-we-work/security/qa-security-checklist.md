...
* Make sure the applications always uses HTTPS. Check that, in the server configuration, the only opened port is `443`. Check it using `nmap` to scan all open ports on your server’s IP e.g. `nmap website.com`. To check specific things read [this](https://www.cyberciti.biz/tips/linux-scanning-network-for-open-ports.html). Another option might be checking some of the ports on this [website](http://canyouseeme.org/).
...
More details can be found [here](https://www.owasp.org/index.php/Testing_for_Sensitive_information_sent_via_unencrypted_channels_(OTG-CRYPST-003)).
...
* Use tools like [snyk](https://snyk.io/) to determine any possible vulnerabilities in the app’s packages. Other tools can be found [here](https://techbeacon.com/13-tools-checking-security-risk-open-source-dependencies-0).
...
* Limited or delayed responses for repeatedly failed login attempts. Also, make sure all failed login attempts are logged and alert the administrator for credential stuffing or brute force.
...
* The app is built in a frameworks that automatically escape XSS by design ( ReactJS, RoR )
...
Other particularities can be found at this [link](https://www.owasp.org/index.php/Testing_for_Clickjacking_(OTG-CLIENT-009)).
